...
class TestDetectPackageVersion:
    def test_detect_package_version_from_package_directory(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_detect_package_version_invalid_version(self, tmp_path: Path) -> None:
        ...
        (pkg_dir / "__init__.py").write_text(
            '__version__ = "invalid.version"', encoding="utf-8"
        )
...
class TestResolveSource:
    ...
    @patch("importlib.util.find_spec")
    def test_resolve_source_module_name_with_submodule_locations(
        self, mock_find_spec, tmp_path: Path
    ) -> None:
    ...
    def test_resolve_source_module_no_origin_or_locations(self, mock_find_spec) -> None:
        ...
        with pytest.raises(
            typer.BadParameter, match="Could not determine location of module"
        ):
...
class TestExtractCommand:
    ...
    def test_extract_package_directory_auto_detect(self, tmp_path: Path) -> None:
        ...
        result = self.runner.invoke(
            app, ["extract", str(pkg_dir), "--out", str(out_dir), "--commit", "abc123"]
        )
    ...
    def test_extract_nonexistent_path(
        self, mock_run_pipeline, mock_resolve_source
    ) -> None:
        """Test extracting from a non-existent path."""
        mock_resolve_source.side_effect = typer.BadParameter(
            "Could not find module or path: nonexistent"
        )
