    ...
    for search_dir in search_dirs:
        ...
        for entry in search_dir.iterdir():
            if entry.is_dir() and (entry / "__init__.py").is_file():
                ...
                if entry.name not in ("__pycache__", ".git", ".venv", "venv", "env", "tests", "docs", "examples"):
...
def _resolve_source(source: str) -> tuple[Path, str | None]:
    ...
    try:
            ...
            if module_path.name == "__init__.py":
                ...
                if '.' in source:
                    ...
                    for _ in range(levels):
                        ...
                        if not (parent / "__init__.py").exists() and parent.name != package_name:
                            break
                        root_path = parent
                ...
                while current.parent != current:
                    if current.name == package_name and (current / "__init__.py").exists():
                        root_path = current
                        break
                    ...
                    if parent.name in ("site-packages", "dist-packages", "Lib"):
                        ...
                        if pkg_in_site.exists() and (pkg_in_site / "__init__.py").exists():
        ...
        # Verify that we found a valid package directory
...
def extract(
    ...
    # Use provided package name or auto-detected one
    if package is None:
        package = default_pkg
    
    # Final attempt to detect package name if still None
    if package is None:
        package = _detect_package_name(root_path)
    ...
    if version is None:
        version = _detect_package_version(root_path, package)
    
    if version is None:
        typer.echo("Could not determine package version. Please specify with --version", err=True)
        raise typer.Exit(1)
    
    typer.echo(f"Extracting API context for {package} v{version}...")
    ...
    if result.ok:
        typer.echo(f"✓ Successfully extracted API context for {package} v{version}")
    ...
    typer.echo(f"✗ Extraction failed with {len(errors)} error(s):", err=True)
    for error in errors:
        typer.echo(f"  {error.code}:{error.path}:{error.message}", err=True)
    raise typer.Exit(1)
